...
void pPb_XKRatio(){
  ...
  for(int c = 0; c<nc; c++ ){
    l[c] = (TList*)f->Get(Form("Xi_toKRatio_%s", sc[c].Data()));
...
//=============================================================================
  ...
  SetStyle(kTRUE);
  gStyle->SetErrorX(0);
  auto can(MakeCanvas("pPb_XKRatio_wCent", 1000, 350));
//=====Make pads===============================================================
 ...
 // dfly = 3e-6; dfuy=4e-1;
  ...
  auto leg(new TLegend(0.50, 0.70, 0.9, 0.95)); SetupLegend(leg);
  leg->AddEntry(h[0][0], "Inclusive", "P")->SetTextSizePixels(20);
  leg->AddEntry(h[1][0], "Perp. cone", "P")->SetTextSizePixels(20);
  leg->AddEntry(h[2][0], "Particle in jets", "P")->SetTextSizePixels(20);
