...
//=============================================================================
  ...
  for(Int_t i = 0; i<4; i++){
    h[i]  = (TH1D*)l[i]->FindObject(Form("InclCen"));
    hE[i] = (TH1D*)l[i]->FindObject(Form("InclErr"));
    gE[i] = (TGraphErrors*)l[i]->FindObject(Form("Inclerr"));
  }
...
//=============================================================================
  ...
  auto can(MakeCanvas("fpPb_Incl_Kshort_Cent", 700, 600));
  ...
  tex->DrawLatex(0.16, 0.1, "K^{0}_{S}, |#eta_{K^{0}_{S}}| < 0.75");
  can->cd();
  padB->cd();
  dfly = 0.1, dfuy = 3.6;
