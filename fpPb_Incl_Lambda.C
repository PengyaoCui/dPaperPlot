...
void fpPb_Incl_Lambda(){
//=============================================================================
  auto f = TFile::Open("./data/pPb.root", "read");
  auto l = (TList*)f->Get(Form("Lambda_sum_0100"));
  f->Close();
  ...
  for (auto i=0; i<nm; ++i) g[i] = new TGraph(Spectrum(i, sd, "Lambda"));
  //for (auto i=0; i<nm; ++i) for (auto j=0; j<4; ++j) g[j][i]->SetLineStyle(9);

//=============================================================================
  auto dflx(0.), dfux(12.);
  auto dfly(1e-6), dfuy(4e1);
  ...
  auto can(MakeCanvas(Form("pPb_Incl_Lambda")));
  can->SetLogy();
  ...
  tex->DrawLatex(0.16, 0.80, Form("Inclusive #Lambda + #bar{#Lambda}"));
