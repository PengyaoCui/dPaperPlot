...
void fpPb_Incl_Kshort(){
//=============================================================================
  auto f = TFile::Open("./data/pPb.root", "read");
  auto l = (TList*)f->Get(Form("Kshort_0100"));
  f->Close();
  ...
  for (auto i=0; i<nm; ++i) g[i] = new TGraph(Spectrum(i, sd, "Kshort"));
  //for (auto i=0; i<nm; ++i) for (auto j=0; j<4; ++j) g[j][i]->SetLineStyle(9);

//=============================================================================
  auto dflx(0.), dfux(12.);
  auto dfly(3e-6), dfuy(6e1);
  ...
  auto can(MakeCanvas(Form("pPb_Incl_Kshort")));
  can->SetLogy();
  ...
  tex->DrawLatex(0.16, 0.80, Form("Inclusive K^{0}_{S}, |#eta_{K^{0}_{S}}| < 0.75"));
