...
void fpPb_JE_LKRatio(){
//=============================================================================

  TList* l; TFile *f;
  f = TFile::Open("./data/pPb.root", "read");
  l = (TList*)f->Get(Form("Lambda_sum_toKRatio_0100"));
  f->Close();
  ...
  for (auto i=0; i<nm; ++i){ 
    hm[i] = (TH1D*)(RatioLK(i, sd, sj, "JC04", "PC04"));
    g[i] = new TGraph(hm[i]);
...
//=============================================================================
  ...
  auto can(MakeCanvas("pPb_JE_LKRatio", 700, 600));
