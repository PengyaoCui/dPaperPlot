...
//=============================================================================
  ...
  for(Int_t i = 0; i<4; i++){
    h[i]  = (TH1D*)l[i]->FindObject(Form("UECen"));
    hE[i] = (TH1D*)l[i]->FindObject(Form("UEErr"));
    gE[i] = (TGraphErrors*)l[i]->FindObject(Form("UEerr"));
  }
...
//=============================================================================
  ...
  auto can(MakeCanvas("fpPb_UE_Lambda_Cent", 700, 600));
  ...
  tex->DrawLatex(0.16, 0.30, "#Lambda + #bar{#Lambda} in perp. cone");
  tex->DrawLatex(0.16, 0.20, "Jet: anti-#it{k}_{T}, #it{R} = 0.4, #it{p}_{T, jet}^{ch} > 10 GeV/#it{c}, |#eta_{jet}| < 0.35");
  tex->DrawLatex(0.16, 0.10, "#it{R}(#Lambda, jet) < 0.4, |#eta_{#Lambda}| < 0.75");
  can->cd();
  padB->cd();
  dfly = 0.1, dfuy = 1.9;
