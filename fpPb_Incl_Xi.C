...
//=============================================================================
  ...
  for (auto i=0; i<nm; ++i) g[i] = new TGraph(Spectrum(i, sd, "Xi"));
  //for (auto i=0; i<nm; ++i) for (auto j=0; j<4; ++j) g[j][i]->SetLineStyle(9);

//=============================================================================
  auto dflx(0.), dfux(12.);
  auto dfly(2e-6), dfuy(8e-1);
  ...
  auto can(MakeCanvas(Form("pPb_Incl_Xi")));
  can->SetLogy();
  ...
  tex->DrawLatex(0.16, 0.80, Form("Inclusive #Xi^{-} + #bar{#Xi}^{+}"));
